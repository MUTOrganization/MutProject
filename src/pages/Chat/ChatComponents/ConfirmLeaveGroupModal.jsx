...
export default function ConfirmLeaveGroupModal({ isOpen, onClose, currentChatRoom, isLoading, onLeave }) {
    return(
        <Modal isOpen={isOpen} onClose={onClose}>
            <ModalContent>
            <ModalHeader className="flex items-center justify-start gap-3 border-b border-gray-200">
                    <h2 className="text-xl font-bold text-gray-800 tracking-tight">ออกจากกลุ่ม</h2>
                    <p className="text-sm text-gray-500">{currentChatRoom?.name}</p>
                </ModalHeader>
                <ModalBody className="flex flex-col justify-center items-center">
                    <div className="flex flex-col items-center justify-center gap-4">
                        <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center">
                            <FaRunning className="w-8 h-8 text-red-500" />
                        </div>
                        <p className="text-red-600 font-medium text-center">คุณต้องการออกจากกลุ่มนี้หรือไม่?<br /></p>
